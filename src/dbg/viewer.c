...
static void	compositeur_titre(Dbg *dbg) {
	...
	render_lg_col(dbg, 2, REG_COL + 1, DBG_ENTETE_REG);
}
...
static void	compositeur_microcodes(Dbg *dbg, t_mcseq *mseq, int pos) {
	...
	render_set_line(dbg, 20, DBG_FOND_BK);
	// Microcode précédent
	render_lg_col(dbg, 20, 3, DBG_MICROCODE_P(microcode_p));
	// Séparateur
	render_append_to_line(dbg, 20, DBG_MICROCODE_S(microcode_p));
	// liste des microcodes suivants
	while (i < mseq->nb_microcodes) {
		microcode = mseq->sequence[i];
		render_append_to_line(dbg, 20, DBG_MICROCODE(i == pos, microcode));
		i++;
	}
	...
	render_set_line(dbg, 21, DBG_FOND_BK);
	render_lg_col(dbg, 21, 3, DBG_LMICROCODE(mseq->sequence[pos], dbg->texte.microcode));
}
...
static void	compositeur_instruction(Mini_ordi *pico, Dbg *dbg, int phase, int pos) {
	...
	render_set_line(dbg, 22, DBG_FOND_BK);
	render_lg_col(dbg, 22, 3, DBG_LINSTRUCTION(dbg->texte.instruction));
}

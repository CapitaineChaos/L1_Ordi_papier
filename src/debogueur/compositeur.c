...
Fichier     : debogueur/compositeur.c
...
void	compositeur_top_menu(Mini_ordi *pico, Dbg *dbg) {
	render_set_line(dbg, DBG_MENU_LINE, DBG_FOND_BK);
	render_lg_col(dbg, DBG_MENU_LINE, 4, DBG_TOP_MENU(IO_STDIN_AVAILABLE,
			IO_OUTPUT_AVAILABLE));
	render_set_line(dbg, DBG_TITLE_LINE, DBG_TOP_TITRE);
}
